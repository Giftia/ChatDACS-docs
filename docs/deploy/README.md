...
8. 避免在程序界面的窗口中点击或者拖动，否则会由于触发Windows命令行窗口的文字选择，导致程序时停。如果观察到程序左上角出现了 `选择` 字样，说明已经进入了时停，在时停期间的所有请求都会阻塞，请在窗口内 `黑色背景区域` 右键一下以退出时停，才可以继续运行。如果程序时停过长，解除时停后会将所有时停期间的消息进行瞬间处理，有可能会导致处理量过大导致小夜猛烈响应输出。这种时候建议别解除时停了，直接重开，右上角 X 掉，重新启动。
:::
